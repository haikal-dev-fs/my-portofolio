...
export async function POST(request: NextRequest) {
  try {
    console.log('Contact form submission started (Supabase version)');
    ...
    console.log('Message inserted successfully into Supabase:', data?.id);
...
export async function GET(request: NextRequest) {
  try {
    // This endpoint is for admin to retrieve messages from Supabase
    const url = new URL(request.url);
    const page = parseInt(url.searchParams.get('page') || '1');
    const limit = parseInt(url.searchParams.get('limit') || '10');
    const offset = (page - 1) * limit;

    const { data: allMessages, error, count } = await supabase
      .from('messages')
      .select('*', { count: 'exact' })
